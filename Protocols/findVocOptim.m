function [struct_voc, Voc] = findVocOptim(struct, approxVoc)
    ...
    %
    % Other m-files required: df, df_analysis, stabilize
    % Subfunctions: IgiveCurrentForVoltage
    % MAT-files required: none
    %
    % See also df, findVoc, findVocDirect, df_analysis.
    ...
    originalCurrent = df_analysis.calcJ(struct);
    ...
    residualJ = df_analysis.calcJ(struct_voc);
...
function [minimizeMe, current, struct_newVapp] = IgiveCurrentForVoltage(struct, Vapp)
    ...
    J = df_analysis.calcJ(struct_newVapp);
