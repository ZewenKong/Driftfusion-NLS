function spvdat = spvana(spvsol)
    ...
    rho.ill = df_analysis.calcrho(spvsol.ill, "whole");
    sigmat.ill = trapz(x, rho.ill, 2);

    % Get illuminated field
    F.ill = df_analysis.calcF(spvsol.ill, "whole");
    % Field at final position x=d as a function of time
    Ft.ill = F.ill(:, end);
    % Ionic component of field
    Fion.ill = df_analysis.calcFion(spvsol.ill);
    ...
    % Calculate Vion
    Vion.ill = df_analysis.calcVion(spvsol.ill);
    % Vion at x=d
    Viont.ill = Vion.ill(:, end);

    %% Dark decay time step
    % integrated charge density as a function of time
    rho.dk = df_analysis.calcrho(spvsol.dk, "whole");
    sigmat.dk = trapz(x, rho.dk, 2);

    % Get dark field
    F.dk = df_analysis.calcF(spvsol.dk, "whole");
    % Field at final position x=d as a function of time
    Ft.dk = F.dk(:, end);
    % Ionic component of field
    Fion.dk = df_analysis.calcFion(spvsol.dk);
    ...
    % Calculate Vion
    Vion.dk = df_analysis.calcVion(spvsol.dk);
    % Vion at x=d
